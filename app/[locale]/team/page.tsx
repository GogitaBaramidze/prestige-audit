...
export const TeamCard = React.memo(
  ({
    ...
    <div className="group relative aspect-[4/5] w-[95%] mx-auto cursor-pointer overflow-hidden rounded-2xl bg-gray-900">
      ...
      {/* Always-visible name — fades out on hover */}
      ...
      {/* Curtain — invisible until hover, then slides in from left as frosted glass */}
      <div
        className="
          absolute inset-0
      ...
      >
        ...
        >
          ...
          >
            <div className="flex items-center gap-5 rounded-full bg-white px-5 py-2 text-xs font-bold text-[#0a1a3f] transition-colors duration-200 hover:bg-cyan-300">
              {viewProfileLabel}
...
export default function TeamPage() {
  ...
  const filtered = TEAM_MEMBERS.filter((m) => m.department === activeFilter);
    ...
    <div className="min-h-screen bg-background">
      ...
      >
        ...
        <div className="mx-auto max-w-7xl relative z-10 px-6">
          <motion.div
            className="inline-flex items-center gap-5 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md text-blue-200 text-xs font-bold uppercase tracking-widest mb-8"
                ...
                <AnimatedCard key={member.id} index={index}>
                  <TeamCard
                    ...
                    departmentLabel={getDeptLabel(
                      member.department as DepartmentKey,
                    )}
                    image={member.image}
                    slug={member.id}
